...
if $ADMIN_LOCAL; then
  ...
  (cd "$REPO_ROOT/admin" && npm install && npm run build)
...
echo "PocketBase:   ${PB_ON_PI:+local }$PB_HOST"
echo "Admin UI:     $([ "$ADMIN_LOCAL" = true ] && echo "local (systemd) or remote as chosen")"
echo "Viewer:       systemd service 'frame-viewer'"
echo "Config:       $VIEWER_CONFIG"
echo "Cache:        $VIEWER_CACHE"
echo "Logs:         journalctl -u frame-viewer -f"
echo "If GL driver changed, reboot recommended."
